...
class StampIdentifier {
        ...
        this.viewportObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                ...
                if (entry.isIntersecting) {
                    ...
                    // CRASH FIX: Unload images when they are no longer intersecting
    ...
    // CRASH FIX: New, more aggressive image unloading logic
    unloadImageIfFaraway(stampElement) {
        ...
        if (img && img.src && img.src.startsWith('blob:')) {
            ...
            // Unload if the stamp is more than two screen heights away (above or below)
            const isFarAway = rect.bottom < -viewportHeight * 2 || rect.top > viewportHeight * 3;
